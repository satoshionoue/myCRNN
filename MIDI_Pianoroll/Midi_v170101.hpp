...
inline string vnum(int i){
	...
	b0=i;
	ss.str("");
	if(b3!=0){ss<<ch(128+b3)<<ch(128+b2)<<ch(128+b1)<<ch(b0);
	}else if(b2!=0){ss<<ch(128+b2)<<ch(128+b1)<<ch(b0);
	}else if(b1!=0){ss<<ch(128+b1)<<ch(b0);
	}else{ss<<ch(b0);
	}//endif
	return ss.str();
}//end vnum
...
inline void tnumAdd(vector<int> &mes,int MM){
	int i=60*1000000/MM;
	mes.push_back(i/65536); i=i%65536;//65536=16^4
	mes.push_back(i/256); i=i%256;//256=16^2
	mes.push_back(i);
}//end tnumAdd
...
public:
	bool operator()(const MidiMessage& a, const MidiMessage& b){
		if(a.time < b.time){
			...
			if(a.isEffNoteOff() && b.isEffNoteOn()){
				return true;
			}else if( a.mes[0]>=192 && a.mes[0]<208 && (b.isEffNoteOff() || b.isEffNoteOn()) ){
				return true;
			}else{
				return false;
			}//endif
...
ofs<<"\n";
	...
	void SetStrData(int TPQN_=2520,int tempo_=120,int num_=4,int den_=4){
		...
		for(int i=0;i<16;i+=1){
			if(programChangeData[i]==0){continue;}
			ss1<<ch(0)<<ch(192+i)<<ch(programChangeData[i]);
			nBytes+=3;
		}//endfor i

		for(int i=0;i<content.size();i+=1){
			content[i].tick=int((tempo*TPQN*content[i].time)/60.);
		}//endfor i
