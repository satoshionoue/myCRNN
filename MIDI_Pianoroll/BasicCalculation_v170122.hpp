...
inline void SetSeedFromTime(){
	//秒単位で変わるので注意
	mt_=mt19937_64((unsigned int)time(NULL));
}//end SetSeedFromTime
...
class MorePair{//-> decreasing order
public:
...
class LessPair{//-> increasing order
public:
	bool operator()(const Pair& a, const Pair& b){
		if(a.value < b.value){
			return true;
...
inline void SmoothVector(vector<double> &p, double smoother){
	for(int i=0;i<p.size();i++){p[i]+=smoother;}
	Normalize(p);
}//end SmoothVector
...
inline vector<string> SplitStr(string str, char sep){
	vector<string> res;
	stringstream ss(str);
	string buf;
