...
inline int TonicToKeyDegree(int tonic){
	return ((7*tonic+1200)%12+5)%12-5;
}//end TonicToKeyDegree
...
inline int KeyToKeyFifth(string key){
//key = F#m -> -3
	string mode="major";
	if(key[key.size()-1]=='m'){
		mode="minor";
	...
	int key_fifth=SitchToPytchClass(key+"4");
	if(mode=="minor"){key_fifth-=3;}
	return key_fifth;
}//end KeyToKeyFifth
...
//cout<<"fullname,root,form,bass:\t"<<fullname<<"\t"<<root<<"\t"<<form<<"\t"<<bass<<endl;
	...
	void Transpose(int tonicTo){//new tonic = prev tonic + tonicTo
		if(root=="R"){return;}
